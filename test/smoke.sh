...
chk $? "session started with orch + principal"
...
# --------------------------------------------------------------------------
printf '\nsession:\n'
"$PIPELINE" tell orch "$REQUEST [SIGNAL:KICKOFF]" --session "$SESSION" >/dev/null 2>&1
chk $? "kickoff delivered to orch"
...
while [ "$waited" -lt 240 ]; do
  ...
  if [ -f "$STATE/awaiting-developer" ]; then
    ...
    case "$req" in
      ...
      plan:*)
        f="${req#plan:}"
        "$PIPELINE" tell orch "Plan and tier approved. [SIGNAL:DEV_APPROVE_PLAN feature=$f]" \
          --session "$SESSION" >/dev/null 2>&1 ;;
    esac
  fi
  sleep 2
...
if [ -f "$STATE/session-complete" ]; then
  ...
  printf '\n--- orch actions ---\n'; tail -30 "$STATE/actions-orch.log" 2>/dev/null
...
grep -q 'EVIDENCE_INVALID F002-off-by-one reason=stale-sha' "$STATE/actions-orch.log" 2>/dev/null
chk $? "stale evidence SHA was detected and the approval refused"
grep -q 'EVIDENCE_VALID F002-off-by-one' "$STATE/actions-orch.log" 2>/dev/null
...
grep -q 'DROPPED_SIGNAL recovered verdict=WORK_APPROVED for F001-config-file' \
  "$STATE/actions-orch.log" 2>/dev/null
...
grep -q 'TEST_MODE_STOP no PR created' "$STATE/actions-orch.log" 2>/dev/null
chk $? "orch stopped in test mode and reported the manual follow-up"
...
before="$(count_in 'RECV' "$STATE/actions-principal.log")"
p_pane="$(jq -r '.agents.principal.pane' "$STATE/registry.json")"
tmux send-keys -t "$p_pane" -l -- "[SIGNAL:FINAL_REVIEW_READY]" 2>/dev/null
tmux send-keys -t "$p_pane" Enter 2>/dev/null
sleep 2
after="$(count_in 'RECV' "$STATE/actions-principal.log")"
[ "$before" -eq "$after" ]
chk $? "unauthenticated signal ignored (A5)"
grep -q 'IGNORED_UNAUTHENTICATED' "$STATE/actions-principal.log" 2>/dev/null
...
sent_id="$(jq -r 'select(.kind=="tell" and .alias=="principal") | .msg_id' \
  "$STATE/events.jsonl" 2>/dev/null | tail -1)"
token="$(cat "$STATE/token")"
before_dedupe="$(count_in 'DEDUPED' "$STATE/actions-principal.log")"
before_recv="$(count_in 'RECV' "$STATE/actions-principal.log")"
tmux send-keys -t "$p_pane" -l -- \
  "[PIPELINE:$token msg=$sent_id] replay [SIGNAL:FINAL_REVIEW_READY]" 2>/dev/null
tmux send-keys -t "$p_pane" Enter 2>/dev/null
sleep 2
after_dedupe="$(count_in 'DEDUPED' "$STATE/actions-principal.log")"
after_recv="$(count_in 'RECV' "$STATE/actions-principal.log")"
...
"$PIPELINE" tell orch "stale verdict arriving late [SIGNAL:WORK_APPROVED feature=F001-config-file]" \
  --session "$SESSION" >/dev/null 2>&1
sleep 2
...
tmux send-keys -t "$p_pane" Enter 2>/dev/null
sleep 1
out="$("$PIPELINE" tell principal "this should be refused" --session "$SESSION" 2>&1)"
rc=$?
...
"$PIPELINE" tell orch "resume after crash [SIGNAL:WORK_APPROVED feature=F001-config-file]" \
  --session "$SESSION" >/dev/null 2>&1
sleep 3
...
PIPELINE_ALIAS=principal PIPELINE_DIR="$STATE" TMUX_PANE="$p_pane" TMUX=1 \
